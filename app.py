...
def appointment_list(doctor_id):
    ...
    query = """
        ...
        WITH r, p, datetime(r.date + 'T' + r.time) AS dateTime
        RETURN r.id AS rendezvous_id, r.date AS date, r.time AS time, p.name AS patient_name ,p.id as patient_id
...
def list_treatments():
    ...
    return render_template('treatment_list.html', treatments=treatments)

# creer traitement 

@app.route('/create_treatment', methods=['GET', 'POST'])
def create_treatment():
    if request.method == 'POST':
        name = request.form['name']
        propriete = request.form['propriete']
        treatment_model.create_treatment(name, propriete)
...
def update_treatment(treatment_id):
    treatment = treatment_model.get_treatment_by_id(treatment_id)
    if not treatment:
        flash("Traitement introuvable.", "error")
        return redirect(url_for('list_treatments'))

    if request.method == 'POST':
        name = request.form['name']
        propriete = request.form['propriete']
        # Appelle de la méthode pour mettre à jour le traitement
        treatment_model.update_treatment(treatment_id, name, propriete)
        flash("Traitement mis à jour avec succès.", "success")
        return redirect(url_for('list_treatments'))

    return render_template('update_treatment.html', treatment=treatment)



@app.route('/delete_treatment/<treatment_id>', methods=['POST'])
def delete_treatment(treatment_id):
    treatment_model.delete_treatment(treatment_id)
    return redirect(url_for('list_treatments'))
...
def get_patient_pathology():
    ...
    for patient in patient_list:
        # Récupère les pathologies et traitements pour le patient
        pathologies = appointment_model.get_pathologies_for_patient(patient["id"])

        if pathologies:
            for pathology in pathologies:  # Boucle sur chaque pathologie
                treatments = appointment_model.get_treatments_for_pathology(pathology["id"])

                if treatments:  # Si des traitements existent pour la pathologie
                    for treatment in treatments:  # Boucle sur chaque traitement
                        resultats.append({
    ...
    print("Résultats récupérés :", resultats)  # Ajoute une vérification dans la console
    return render_template('patient_diagnostic.html', resultats=resultats)

@app.route('/patient/<patient_id>')
def patient_details(patient_id):
    # Obtenez les détails du patient
    patient_info = patient_model.get_patient_by_id(patient_id)  # Fonction pour récupérer les informations du patient
    # Obtenez les pathologies associées à ce patient
    pathologies = appointment_model.get_pathologies_for_patient(patient_id)  # Fonction pour récupérer les pathologies

    return render_template('patient_details_list.html', patient=patient_info, pathologies=pathologies)

@app.route('/add_pathology/<patient_id>', methods=['GET', 'POST'])
def add_pathology_to_patient(patient_id):
    if request.method == 'GET':
        # Récupère toutes les pathologies pour les afficher dans la liste déroulante
        pathologies = appointment_model.get_all_pathologies()  # Fonction qui récupère toutes les pathologies
        return render_template('add_pathology_form.html', patient_id=patient_id, pathologies=pathologies)

    elif request.method == 'POST':
        # Lorsque le formulaire est soumis, récupérer le `pathology_id` sélectionné
        pathology_id = request.form['pathology_id']
        # Ajouter la pathologie au patient en créant une relation
        result = patient_model.add_pathology_to_patient(patient_id, pathology_id)
        if result:
            return redirect(url_for('patient_details', patient_id=patient_id))  # Rediriger vers la page du patient
        else:
            return "Erreur lors de l'ajout de la pathologie", 400
