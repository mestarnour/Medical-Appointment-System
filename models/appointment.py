class AppointmentModel:
    ...
    def check_appointment_exists(self, patient_id, doctor_id, date,time):
        query = """
        MATCH (r:RendezVous {date: date($date),time:time($time)})<-[:PLANIFIE]-(d:Medecin {id: $doctor_id})
        RETURN r
        """
    ...
    def create_appointment(self, patient_id, doctor_id, date,time):
        ...
        CREATE (p)-[:A_PRIS]->(r:RendezVous {date: date($date),time:time($time)})<-[:PLANIFIE]-(d)
        RETURN r
        """
    ...
    def get_pathologies_for_patient(self, patient_id):
        ...
        """
        result = self.db.query(query, {"patient_id": patient_id})
        return result
    ...
    def get_all_pathologies(self):
       query = "MATCH (pa:Pathologie) RETURN pa.id as id ,pa.name as name ,pa.propriete as propriete"
       result = self.db.query(query)
       return result  # Retourne toutes les pathologies
